"""Run one approved YOLO26n v5 experiment and record its exact environment."""
...
DATASET = ROOT / "dataset/dataset-v5"
DATA = DATASET / "dataset.yaml"
AUDIT = DATASET / "metadata/audit.json"
...
def main():
    ...
    if audit.get("status") != "passed" or not DATA.is_file():
        raise SystemExit("the audited v5 dataset is missing")
    ...
    command = f".venv-export/bin/python -m scripts.training_v5.train {experiment}"
    if seed != 42:
        command += f" {seed}"
    receipt = {
