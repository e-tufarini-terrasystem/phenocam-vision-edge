"""Export the frozen v5 selection and verify its ONNX runtime contract."""
...
MODEL_PT = ROOT / "models/yolo26n-v5.pt"
MODEL_ONNX = ROOT / "models/yolo26n-v5.onnx"
MODEL_RECEIPT = ROOT / "models/yolo26n-v5.json"
...
def main():
    ...
    evidence = tuple(
        WORK / f"final/{split}/{kind}/{kind}-metrics.json"
        for split in ("pklot_holdout", "test_id", "test_ood")
        for kind in ("standard", "runtime")
    )
    if not freeze_path.is_file() or any(not path.is_file() for path in evidence):
        raise SystemExit("freeze and complete all one-time sealed evaluations before export")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    checkpoint = Path(freeze["checkpoint"]).resolve()
    if (
        freeze.get("test_status") != "opened"
        or not checkpoint.is_file()
        or not checkpoint.is_relative_to(WORK.resolve())
        or sha256(checkpoint) != freeze["model_sha256"]
    ):
        raise SystemExit("frozen model is missing or changed")
    for path in evidence:
        report = json.loads(path.read_text(encoding="utf-8"))
        if report.get("model_sha256") != freeze["model_sha256"]:
            raise SystemExit("sealed evaluation does not match the frozen model")
    if any(path.exists() for path in (MODEL_PT, MODEL_ONNX, MODEL_RECEIPT)):
        raise SystemExit("a final v5 model artifact exists; refusing to overwrite it")

    with tempfile.TemporaryDirectory(prefix=".v5-export-", dir=WORK) as directory:
        temporary_pt = Path(directory) / "selected.pt"
        shutil.copy2(checkpoint, temporary_pt)
        exported = Path(YOLO(temporary_pt).export(
        ...
        if (
            (width, height) != (freeze["imgsz"], freeze["imgsz"])
            or any(names.get(key) != value for key, value in EXPECTED.items())
        ):
    ...
    portable_selection = dict(freeze)
    portable_selection["checkpoint"] = str(checkpoint.relative_to(ROOT))
    portable_replicas = []
    for replica in freeze.get("leave_one_seed_out", []):
        replica_checkpoint = Path(replica["checkpoint"]).resolve()
        if not replica_checkpoint.is_relative_to(WORK.resolve()):
            raise RuntimeError("stability checkpoint is outside the v5 workspace")
        portable_replica = dict(replica)
        portable_replica["checkpoint"] = str(replica_checkpoint.relative_to(ROOT))
        portable_replicas.append(portable_replica)
    portable_selection["leave_one_seed_out"] = portable_replicas

    receipt = {
        "selection": portable_selection,
        "source_checkpoint": str(checkpoint.relative_to(ROOT)),
        "pt": str(MODEL_PT.relative_to(ROOT)), "pt_sha256": sha256(MODEL_PT), "pt_bytes": MODEL_PT.stat().st_size,
        "onnx": str(MODEL_ONNX.relative_to(ROOT)), "onnx_sha256": sha256(MODEL_ONNX), "onnx_bytes": MODEL_ONNX.stat().st_size,
        ...
        "benchmark": "20 zero-tensor ONNX CPU runs after 3 warmups on the recorded development host; not a Raspberry Pi benchmark",
