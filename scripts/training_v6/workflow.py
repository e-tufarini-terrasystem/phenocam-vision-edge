...
from .preflight import CONFIG, ROOT, WORK, sha256, write


def command(stage, module, *arguments):
    args = [sys.executable, "-m", f"scripts.training_v6.{module}", *map(str, arguments)]
...
def evaluation(name, pt, onnx, thresholds=None, split="val", frozen=None, include_pt_runtime=False):
    ...
    for kind, model in (("pt", pt), ("onnx", onnx)):
        ...
        command(f"{split}-{name}-{kind}", "evaluation", *args)
...
def main():
    ...
    for recipe in config["recipes"]:
        training(recipe, 42)
    for name, filename in (("base", "yolo26n"), ("v5", "yolo26n-v5")):
        evaluation(name, ROOT / f"models/{filename}.pt", ROOT / f"models/{filename}.onnx")
    for recipe in config["recipes"]:
    ...
    models = {"base": (ROOT / "models/yolo26n.pt", ROOT / "models/yolo26n.onnx"),
              "v5": (ROOT / "models/yolo26n-v5.pt", ROOT / "models/yolo26n-v5.onnx"),
              "v6": (ROOT / frozen["checkpoint"], ROOT / frozen["onnx"])}
    # PT/ONNX comparison covers the entire validation at the frozen threshold.
    command("val-v6-pt-fixed", "evaluation", "--model", models["v6"][0], "--name", "v6-pt-fixed",
