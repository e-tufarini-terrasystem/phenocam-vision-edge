...
def load(name, threshold=None):
    ...
    if name in {"base", "v5"}:
        ...
        stem = "yolo26n" if name == "base" else "yolo26n-v5"
        expected = {f"{kind}_sha256": preflight["model_hashes"][f"models/{stem}.{kind}"] for kind in ("pt", "onnx")}
    else:
...
def candidate():
    ...
    baselines = {name: load(name) for name in ("base", "v5")}
    candidates = []
...
def acceptance(replicas, baseline, gate):
    ...
    checks = {
        ...
        "no_replica_below_v5": min(gains) + 1e-12 >= gate["minimum_replica_f1_gain"],
...
def freeze():
    ...
    baseline = selected["baselines"]["v5"]
    ...
    for name, row in {"v6": {"confidence": threshold, "pt_sha256": sha256(checkpoint),
