"""Run existing standard and production evaluators against pinned v6 inputs."""
...
from scripts.training_v5.standard import metrics
from .preflight import CONFIG, DATASET, ROOT, WORK, code_hashes, sha256, write
...
def guard_runtime():
    ...
    for name, digest in preflight["code_sha256"].items():
        if name.startswith(("phenocam/", "scripts/runtime_evaluation/", "scripts/training_v5/")):
...
def evaluate(model, name, split="val", thresholds=None, receipt=None, standard=True, runtime=True):
    ...
    dataset = DATASET if split in {"val", "pklot_holdout"} else ROOT / "dataset/dataset-v3"
    ...
    if runtime and not runtime_path.exists():
        ...
        command = [sys.executable, "-m", "scripts.runtime_evaluation.evaluate", "--model", str(model),
