...
base `models/yolo26n.pt`, sull'intero `dataset/dataset-v6/dataset.yaml`.
...
La F1 del seed 42 supera v5 di 0,01463, ma la perdita mAP Open Images è
...
assenza di repliche con F1 sotto v5: la migliore F1 del seed 42 non è stabile
sui primi due seed.
...
| Seed | Precision ONNX | Recall ONNX | F1 ONNX | Differenza F1 vs v5 | mAP Open Images | Recall PhenoCam | Recall PKLot |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
v5 −0,02162 e deviazione standard campionaria 0,03980. Falliscono incremento
...
Confronto del modello consegnato (seed 42, soglia 0,45) con v5 (soglia 0,48).
...
| Classe | F1 v5 | P v6 | R v6 | F1 v6 | mAP v5 | mAP v6 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
classi COCO: mAP50–95 v5 0,52965, v6 0,56973. Le altre classi COCO non
hanno annotazioni nella validation.

| Sorgente | F1 v5 | P v6 | R v6 | F1 v6 | mAP v5 | mAP v6 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
incontaminati. Soglie fissate sulla validation: base 0,45, v5 0,48, v6 0,45.
...
| PKLot holdout | v5 | 6 | 0,81437 | 0,20716 | 0,33030 | 0,09184 |
| PKLot holdout | v6 | 6 | 0,88966 | 0,68774 | 0,77577 | 0,12078 |
| TEST-ID | base | 200 | 0,39746 | 0,56433 | 0,46642 | 0,57809 |
| TEST-ID | v5 | 200 | 0,41525 | 0,55305 | 0,47435 | 0,58256 |
| TEST-ID | v6 | 200 | 0,48124 | 0,49210 | 0,48661 | 0,54575 |
| TEST-OOD | base | 240 | 0,93114 | 0,40901 | 0,56836 | 0,13260 |
| TEST-OOD | v5 | 240 | 0,93506 | 0,41105 | 0,57106 | 0,13421 |
| TEST-OOD | v6 | 240 | 0,88889 | 0,48530 | 0,62783 | 0,13795 |

Su TEST-OOD, separando lo stress estremo `raspberrypi2.local`, F1 v5/v6
...
Il confronto riprende l'evidenza v5: LR 0,001 con adattamento ampio aveva
peggiorato le metriche; v6 verifica un adattamento più ampio con LR conservativo.

Augmentation fissate in `scripts/training_v6/experiments.json`: mosaic 0,5,
...
Si conserva la non-deterministicità residua MPS già osservata in v5.
...
Griglia identica per base, v5 e candidati: 0,20–0,70 con passo 0,01.
...
Accettazione: incremento medio F1 ONNX sui tre seed ≥0,005 contro v5
rimisurato; nessun seed sotto v5; per ciascun seed perdita mAP50–95 Open
Images ≤0,01 e recall PhenoCam/PKLot validation non inferiore a v5.
...
Validation v6 ereditata da v5: 128 Open Images, 75 PhenoCam (solo due box
...
Nel dominio esistente `scripts/training_v6/`: `preflight.py` (~190 linee),
`train.py` (~170), `evaluation.py` (~180), `selection.py` (~160),
`artifacts.py` (~180); stime produttive senza test. `experiments.json`
registra la griglia. Il builder v6 già presente resta invariato.
...
`output/training-v6/` conserva snapshot del codice effettivo, diff iniziale,
...
preesistenti v3/v5 in `provenance/dataset-caches-before-evaluation/`, insieme
all'inventario dei cache generati dal training v6. Servono a preservare anche
...
`output/training-v6/provenance/environment.txt` prima del training.

```sh
python3.13 -m venv .venv-export
export YOLO_AUTOINSTALL=false YOLO_NUM_THREADS=4
.venv-export/bin/python -m pip install -r requirements/export.txt \
  -r requirements/runtime.txt torch==2.12.1 torchvision==0.27.1 \
  scipy==1.18.1 PyYAML==6.0.2 onnx==1.19.0 onnxslim==0.1.71
.venv-export/bin/python -m pip check
.venv-export/bin/python -m scripts.training_v6.preflight
YOLO_NUM_THREADS=4 .venv-export/bin/python -m scripts.training_v6.train head 42
YOLO_NUM_THREADS=4 .venv-export/bin/python -m scripts.training_v6.train neck-head 42
# In alternativa, esegue l'intero protocollo in sequenza e riusa solo run completati:
YOLO_NUM_THREADS=4 caffeinate -i .venv-export/bin/python -m scripts.training_v6.workflow
```

Per ricreare esattamente l'ambiente usare il lock delle dipendenze transitive:
`.venv-export/bin/python -m pip install -r output/training-v6/provenance/environment.txt`.
...
`output/training-v6/attempts/head-seed42-torch2.8/`. Il retry riguarda lo stesso
...
- Dataset ricontrollato integralmente dopo il ripristino dell'ambiente:
  ...
  2.004 immagini v5 e relative annotazioni identiche, test storici esclusi.
- Base PT SHA-256:
...
- `YOLO_AUTOINSTALL=false .venv-export/bin/python -m unittest discover -s tests`:
  ...
  runtime minimale aveva segnalato l'assenza di Ultralytics per i test v5;
...
Monitoraggio riproducibile: `output/training-v6/workflow.json` identifica il
