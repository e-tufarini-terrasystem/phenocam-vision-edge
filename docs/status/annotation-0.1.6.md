# 0.1.6 — primo batch operativo per annotazione
...
> `git cat-file blob 0e13e73a059add5d37fa581f264b52e23974a268` from the
...
Stato: correzioni completate in CVAT e dataset 0.1.6 materializzato. Inclusi 57
...
- Ricostruzione deterministica dello split originale con seed 20260831:
  ...
  su 24 giorni, coerenti con le 1.029 immagini documentate in 0.1.3.
...
- PhenoZero1 dal 10 settembre e TS02 dal 9 settembre non sono candidati train
  in questo batch. Validation e test 0.1.6 devono essere definiti separatamente;
...
Progetto [Phenocam privacy detector 0.1.6](http://localhost:8080/projects/2), ID 2,
...
Proposte dal modello locale ONNX 0.1.5, SHA-256
...
Classi operative e attributi coincidono con il progetto 0.1.3; `ambiguous` resta
...
- Sessanta identità, hash file e hash pixel unici; nessuna corrispondenza con
  l'intero manifest sorgenti 0.1.3, oltre alle esclusioni dei giorni riservati.
...
Esportati COCO e backup completi dei tre task del ciclo 0.1.6, datati
11 settembre 2026. I prefissi esatti sono conservati nel documento originale.
...
- Rapporto visuale HTML (workspace del training 0.1.6 (`review-2026-09-11/review.html`), local evidence).
- Risultati strutturati e hash degli snapshot (workspace del training 0.1.6 (`review-2026-09-11/review.json`), local evidence).
...
riesportare e ricontrollare prima di materializzare 0.1.6. Se un caso resta
...
## Esito finale: CVAT corretto e dataset 0.1.6 creato
...
Tre frame ricevono il tag `ambiguous` e non entrano nella 0.1.6:
...
Export finali e backup in `dataset/workspace/annotation/exports/`, relativi ai
task 18, 19 e 20 del ciclo 0.1.6, datati 11 settembre 2026. I prefissi esatti
sono nel documento originale. Il suffisso `final2` del task 18 include la rifinitura dei bordi
superiori delle tre auto aggiunte. Gli export precedenti sono conservati.

Artefatto: `dataset.yaml` del dataset per il training 0.1.6 (evidenza locale).
...
| --- | ---: |
| Training | 2.020 |
| Validation, identica a 0.1.5 | 206 |
| PKLot holdout, identico a 0.1.5 e fuori dal YAML train/val | 6 |
| Totale | 2.232 |

Le 2.004 immagini 0.1.5 e tutte le loro label sono copiate e verificate, senza
hard link. Aggiunte 57 immagini intere (PhenoZero1 22, PhenoZero2 11, TS02 24)
e 171 crop generati con le regole 0.1.5. Totale 7.119 box, incluse 551 box nelle
nuove immagini intere e 166 nelle nuove viste crop. Le classi COCO e il relativo
ordine restano quelli di 0.1.5. Nessun training è stato avviato.

L'audit (dataset 0.1.6 (`metadata/audit.json`), local evidence) passa i controlli su
...
immagine/label ereditati con 0.1.5 e controlla tutte le 7.119 righe YOLO. Nessun
frame dei test storici è stato aggiunto. Fingerprint 0.1.6:
`e1491ab0e3bc073c167026f7e0dd0057c86bede700baa2122a8d3a4126dbfddc`.

Ricevute e prove locali in
workspace del training 0.1.6 (`corrections-2026-09-11/`): `plan.json`,
...
da preparare prima di misurare il miglioramento operativo di un modello 0.1.6.

## Struttura locale prima dell'esecuzione

### Completamento delle correzioni e materializzazione 0.1.6
...
nessuna sovrascrittura di 0.1.5, validation 0.1.5 identica e test storici esclusi.
...
Struttura prevista prima dell'implementazione, riusando le librerie del builder 0.1.5:

```text
<training workspace 0.1.6>/corrections-2026-09-11/
  corrections.py                  (~150 linee produttive; patch CVAT con readback)
  task-<ID>-before/after.json      (snapshot e provenienza)
<training implementation 0.1.6>/
...
<training configuration 0.1.6>   (hash e regole del batch)
<dataset 0.1.6>/               (artefatto generato, immagini escluse da Git)
```

Si riusano conversione YOLO, crop e audit 0.1.5. Le verifiche sull'artefatto reale
confrontano integralmente i file 0.1.5 ereditati, i conteggi CVAT/YOLO, le esclusioni
`ambiguous`, gli hash e la separazione dei gruppi tra split.
PyYAML, già importato dal builder 0.1.5 ma assente dall'ambiente locale corrente,
...
```text
<training workspace 0.1.6>/          (ignorato da Git)
  ...
  screening/                          (proposte 0.1.5, mai label umane)
